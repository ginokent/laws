...
echo "LAWS_REMOTE_COMMAND    : ${LAWS_REMOTE_COMMAND:="cd ${LAWS_REMOTE_REPOSITORY} && make"}"

# enable print commands
set -x

# upload
rsync -az ${LAWS_LOCAL_REPOSITORY} ${LAWS_REMOTE_USER}@${LAWS_REMOTE_HOST}:${LAWS_REMOTE_REPOSITORY}
